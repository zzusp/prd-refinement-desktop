...
type MaterialInputItem = Pick<MaterialFile,'logicalPath'|'role'|'status'>;
export function summarizeMaterialInputs(files:MaterialInputItem[]){
  const standalone:MaterialInputItem[]=[],groups=new Map<string,MaterialInputItem[]>();
  for(const file of files){
    if(file.role==='primary'||file.status==='excluded')continue;
    const normalized=file.logicalPath.replaceAll('\\','/'),separator=normalized.indexOf('/');
    if(separator<1){standalone.push(file);continue;}
    const directory=normalized.slice(0,separator),items=groups.get(directory)??[];
    items.push(file);groups.set(directory,items);
  }
  return {standalone,directories:[...groups].map(([name,items])=>({name,items}))};
}
...
export function MaterialWorkspace({ onStarted, onBack, mode={kind:'new'} }: { onStarted: (task:AnalysisTask) => void; onBack: () => void; mode?:MaterialWorkspaceMode }) {
  ...
  const [analysisText,setAnalysisText]=useState(''),[draftState,setDraftState]=useState<'idle'|'saving'|'saved'|'error'>('idle'),[detailsOpen,setDetailsOpen]=useState(mode.kind==='adjustment');
  ...
  const inputSummary=summarizeMaterialInputs(bundle?.files??[]);
  ...
  return <div className={`page material-page ${dragging ? 'material-dragging' : ''}`}
    ...
    {!api ? <div className="panel">资料导入需要在桌面应用中使用。</div> : <>
      ...
      {!bundle ? <div className="material-start-shell"><section className="material-intake-card"><div className="upload-mark"><FileText/></div><h2>选择定版 PRD</h2><p>支持 HTML、DOC、DOCX、PDF、Markdown 和 TXT</p><button className="primary" disabled={!!busy} onClick={() => void add('files', undefined, true)}><Upload/>选择文件</button><small>也可以将文件拖到此区域</small></section>{bundles.length>0&&<section className="material-resume"><div><strong>继续准备已有资料</strong><span>打开上次尚未开始分析的资料包。</span></div><select aria-label="已有资料包" disabled={!!busy} value="" onChange={e => void select(e.target.value)}><option value="">选择已有资料</option>{bundles.map(item => <option key={item.id} value={item.id}>{item.name || item.id} · {states[item.state]} · v{item.revision}</option>)}</select></section>}</div> : <>
        ...
        <section className="material-supplements"><header><div><h2>补充资料 <small>可选</small></h2><p>{mode.kind==='adjustment'?'已带入当前版本的补充文件和资料目录，可直接增删或调整用途。':bundle.files.filter(file=>file.role!=='primary').length?`已添加 ${bundle.files.filter(file=>file.role!=='primary').length} 项，可在资料明细中调整用途。`:'可以添加说明文档、原型、图片或整个资料目录。'}</p></div><div className="material-actions"><label>资料用途<select disabled={locked} value={role} onChange={e => setRole(e.target.value as MaterialRole)}><option value="supplement">补充资料</option><option value="historical">历史参考</option></select></label><button className="secondary" disabled={locked} onClick={() => void add('files')}><Plus/>添加文件</button><button className="secondary" disabled={locked} onClick={() => void add('directory')}><FolderOpen/>添加目录</button></div></header>{mode.kind==='adjustment'&&<div className="material-current-inputs" aria-label="当前版本补充资料">{!inputSummary.standalone.length&&!inputSummary.directories.length?<p className="material-current-empty">当前版本没有补充文件或资料目录，可直接添加。</p>:<>{inputSummary.standalone.length>0&&<div className="material-current-group"><strong>补充文件</strong><ul>{inputSummary.standalone.map(file=><li key={file.logicalPath}><FileText/><span>{file.logicalPath}</span><small>{roles[file.role]}</small></li>)}</ul></div>}{inputSummary.directories.length>0&&<div className="material-current-group"><strong>资料目录 <small>按资料包路径</small></strong><ul>{inputSummary.directories.map(directory=><li key={directory.name}><FolderOpen/><span>{directory.name}</span><small>{directory.items.length} 个文件 · {[...new Set(directory.items.map(file=>roles[file.role]))].join('、')}</small></li>)}</ul></div>}</>}</div>}<details className="material-directory-options"><summary>添加目录时的选项</summary><label className="mount-label">资料包内目录名称<input disabled={locked} value={mount} onChange={e => setMount(e.target.value)} placeholder="留空时使用所选目录名"/></label><p>最多 1000 个文件，单文件 100 MiB，总计 500 MiB。</p></details></section>
        <section className="material-analysis-note" aria-labelledby="material-note-title"><header><div><h2 id="material-note-title">补充说明与调整 <small>选填</small></h2><p>{mode.kind==='adjustment'?'已带入当前版本的提示词，可在下方直接修改。':'可以一次描述多个功能的背景、补充、范围调整、细化要求或澄清结论。'}</p></div><span className={`draft-state ${draftState}`} role="status">{draftState==='saving'?'正在保存':draftState==='saved'?`已保存到本机${bundle.analysisDraft?.updatedAt?` · ${new Date(bundle.analysisDraft.updatedAt).toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit'})}`:''}`:draftState==='error'?'保存失败':'尚未填写'}</span></header><label htmlFor="material-analysis-text" className="sr-only">本次分析的补充说明与调整</label><textarea id="material-analysis-text" className="resize-none" rows={6} maxLength={20000} disabled={locked} value={analysisText} onChange={event=>{setAnalysisText(event.target.value);setDraftState('saving');if(error.startsWith('说明保存失败'))setError('')}} placeholder={'例如：\n本期只做查询和合并提醒，自动合并暂不做。\n同名判断以活动名称去除首尾空格后完全一致为准。\n查询部分细化到字段和交互，历史原型只参考布局。'}/><footer><small>{analysisText.length.toLocaleString('zh-CN')} / 20,000 字 · 选择或更换文件不会清空说明</small></footer></section>
