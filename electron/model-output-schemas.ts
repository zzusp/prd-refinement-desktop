...
// 仅保留内部项目结构的空数组，任何节点都不能创建业务澄清。
const noClarifications=z.array(z.never());
export const detailRequirementProposal=o({id:t,text:t,evidenceIds:ids,featureId:t.optional()});
export const requirementResult=o({id:t,featureId:t,text:t,sourceRefs:refs,state,deliveryScope:z.enum(['current','excluded']).optional()});
...
const role=z.enum(['requirement','context','example','summary','out-of-scope']);
...
const issueFields={id:t,direction:z.enum(['forward','reverse','cross']),type:t,sourceUnitIds:ids,affectedIds:ids,detail:t,owner:z.enum(['feature-grouping','requirement-detail','requirement-relation','runtime-output'])};
const category=z.enum(['rule-extraction','feature-boundary','detail-mismatch','unclassified']);
const issueProposal=o({...issueFields,category});
const issueResult=o({...issueFields,category,identityKey:t.optional(),disposition:z.enum(['open','repaired','dismissed']).optional(),repairAttempts:z.number().int().nonnegative().optional(),dependencyHash:t.optional(),closedDependencyHash:t.optional(),aliases:ts.optional()});
...
const detailProposal=o({requirements:z.array(detailRequirementProposal.omit({featureId:true}))});
const detailResult=o({requirements:z.array(requirementResult),clarifications:noClarifications});
...
const operation=o({id:t,quote:t,kind:z.enum(['organization','business-fact','replace-fact','defer','question']),instruction:t,featureIds:ts,atomicGroupId:t.optional()});
const adjustmentPlan=o({operations:z.array(operation)});
const adjustmentReview=o({passed:z.boolean(),issues:ts});
...
const adjustmentActions=o({requirementActions:z.array(requirementAction),relationActions:z.array(relationAction)});
const snapshot=o({feature:featureResult,requirements:z.array(requirementResult),relations:z.array(relationResult)});
const userEvidence=z.object({id:t,author:z.literal('user'),kind:z.enum(['refinement-instruction','supplement']),content:t,createdAt:t,version:z.number().int(),businessFact:z.boolean(),appliesTo:o({scope:z.enum(['feature','all']),featureIds:ts,clarificationIds:noClarifications.optional()})}).passthrough();
const adjustmentInput=z.object({...sources,projectContextHash:t,feature:featureResult,currentRequirements:z.array(detailRequirementProposal),relations:z.array(relationResult),userOpinions:z.array(o({operation,evidence:userEvidence,notice:t}))}).passthrough();
export const projectResultSchema=z.object({id:t,name:t,sourceName:t,sourceHash:t,revision:z.number().int(),importedAt:t,rawText:z.string(),stage:z.enum(['imported','inventory','refining','review']),sourceUnits:z.array(unit),features:z.array(featureResult),requirements:z.array(requirementResult),clarifications:noClarifications,relations:z.array(relationResult).optional()}).passthrough();
function contract<I extends z.ZodType,P extends z.ZodType,R extends z.ZodType>(input:I,proposal:P,result:R){
 const forbidden=new Set(['clarifications','currentClarifications','beforeClarifications','clarificationContextRequirements','acceptedProposals','clarificationActions','clarificationResolutions','resolutionProposal','clarificationDraft']);
 const checkedInput=input.superRefine((value,ctx)=>{
  const visit=(item:unknown,path:(string|number)[])=>{
   ...
   for(const [key,child]of Object.entries(item)){
    if(forbidden.has(key))ctx.addIssue({code:'custom',path:[...path,key],message:'节点输入不允许待处理事项或建议字段'});
    else visit(child,[...path,key]);
   }
  };visit(value,[]);
 });
 return {version:2,input:checkedInput,proposal,result};
}
export const nodeContracts={
 ...
 audit:contract(z.object({...sources,feature:featureHead,requirements:z.array(requirementResult),requirementCatalog:z.array(o({id:t,text:t}))}).passthrough(),o({issues:z.array(issueProposal),relations:z.array(relationProposal)}),o({issues:z.array(issueResult),relations:z.array(relationResult)})),
 repair:contract(z.object({...sourcedInput,features:z.array(featureResult),currentRequirements:z.array(detailRequirementProposal),issues:z.array(issueResult)}).passthrough(),o({requirements:z.array(detailRequirementProposal),deleteRequirementIds:ts}),o({...detailResult.shape,deleteRequirementIds:ts,deleteClarificationIds:noClarifications})),
 repairReview:contract(z.object({...sources,beforeRequirements:z.array(requirementResult),requirements:z.array(requirementResult),originalIssues:z.array(issueResult)}).passthrough(),o({originalIssueResults:reviewResults,introducedIssues:z.array(issueProposal),discoveredIssues:z.array(issueProposal)}),o({originalIssueResults:reviewResults,introducedIssues:z.array(issueResult),discoveredIssues:z.array(issueResult)})),
 adjustmentParse:contract(o({feedback:t,references:z.array(o({kind:z.enum(['feature','requirement']),id:t})),features:z.array(o({id:t,name:t,requirements:z.array(o({id:t,text:t}))}))}),adjustmentPlan,adjustmentPlan),
 ...
 adjustmentReview:contract(z.object({...sources,operations:z.array(operation),userEvidence:z.array(userEvidence),candidate:snapshot}).passthrough(),adjustmentReview,adjustmentReview),
} as const;
