...
export const clarificationResult=z.object({...question,level:z.enum(['blocking','suggestion','ignorable']),sourceRefs:refs,state:z.enum(['open','resolved','dismissed']),defaultResolution:t.optional(),resolutionProposal:o({...resolutionFields,sourceRefs:refs}).optional(),userDecision:o({text:t,confirmedAt:t,status:z.literal('pending-prd-sync'),operationId:t}).optional()}).passthrough().superRefine((q,c)=>{
...
export const nodeContracts={
 ...
 adjustmentParse:contract(o({feedback:t,references:z.array(o({kind:z.enum(['feature','requirement','clarification']),id:t})),acceptedProposals:z.array(o({clarificationId:t,baseRecommendation:t,finalText:t})),features:z.array(o({id:t,name:t,requirements:z.array(o({id:t,text:t}))})),clarifications:z.array(readQuestion)}),adjustmentPlan,adjustmentPlan),
