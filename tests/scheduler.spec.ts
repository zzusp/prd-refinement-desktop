...
import {estimateTokens} from '../electron/prompt-budget';

const root=path.resolve('docs/tmp/test-run/vitest/scheduler',randomUUID());
afterAll(()=>rm(root,{recursive:true,force:true}));
const config:RuntimeConfig={adapter:'codex-oauth',provider:'fake',fastModel:'fast',fastReasoningEffort:'low',model:'model',reasoningEffort:'low',maxParallel:1,maxNodeParallel:3};
...
const input=(prompt:string)=>{const parsed=JSON.parse(prompt.slice(prompt.lastIndexOf('节点输入：')+5));return parsed.previousCandidate?parsed.input:parsed};
...
function runtime(prompts:string[]):AnalysisRuntime{return{start:async()=>{},stop:async()=>{},diagnostics:()=>'',async executeOperation(request){return {completion:'completed',value:JSON.parse(await this.promptAndWait(request.operationId,`${request.instructions}\n节点输入：${JSON.stringify(request.input)}`,request.timeoutMs,request.images,request.submission.parameters))}},promptAndWait:async(_id,prompt)=>{prompts.push(prompt);const value=input(prompt);
  ...
  if(prompt.includes('“功能候选识别”'))return JSON.stringify({features:[{id:'LOCAL-F1',name:'字段维护',kind:'function',appliesToFeatureIds:[],evidenceIds:value.evidenceCatalog.map((evidence:{id:string})=>evidence.id)}],sourceDispositions:value.sourceUnits.map((unit:SourceUnit)=>({sourceUnitId:unit.id,contentRole:'requirement',reason:'明确要求',featureIds:['LOCAL-F1']}))});
  if(prompt.includes('“功能清单统一”'))return JSON.stringify({features:value.candidates.map(({id,name,kind,appliesToFeatureIds}:any)=>({id,name,kind,appliesToFeatureIds})),candidateMappings:value.candidates.map((item:{id:string})=>({candidateId:item.id,featureIds:[item.id]}))});
...
describe('Pipeline 22 调度不变量',()=>{
  it('单功能三批中间批失败后重启只补该批，成功回执不重复需求',async()=>{
    const prompts:string[]=[],directory=path.join(root,'detail-batch-resume'),value=project(),calls=new Map<string,number>(),base=runtime(prompts);
    value.sourceUnits=Array.from({length:25},(_,index)=>({id:`S-${index+1}`,label:`条目${index+1}`,kind:'paragraph' as const,excerpt:`字段${index+1}必须保存。`,location:`第${index+1}段`,status:'processed' as const}));
    value.rawText=value.sourceUnits.map(unit=>unit.excerpt).join('\n');
    const batched:AnalysisRuntime={...base,promptAndWait:async(id,prompt,timeout,images,schema)=>{
      const data=input(prompt);
      if(prompt.includes('“功能清单统一”'))return JSON.stringify({features:[{id:'ALL',name:'字段管理',kind:'function',appliesToFeatureIds:[]}],candidateMappings:data.candidates.map((item:any)=>({candidateId:item.id,featureIds:['ALL']}))});
      if(prompt.includes('“逐功能细化”')){
        const key=data.sourceUnits.map((unit:SourceUnit)=>unit.id).join(','),count=(calls.get(key)??0)+1;calls.set(key,count);
        if(data.sourceUnits.some((unit:SourceUnit)=>unit.id==='S-13')&&count===1)throw new Error('中间批真实失败');
      }
      return base.promptAndWait(id,prompt,timeout,images,schema);
    }};
    const scheduler=new AnalysisTaskScheduler(directory,async()=>config,()=>{},()=>batched);await scheduler.initialize();const created=await scheduler.create(value),failed=await terminal(scheduler);
    expect(failed.status,failed.error).toBe('failed');expect(calls.size).toBe(3);
    expect(Object.values(failed.checkpoint?.nodeReceipts??{}).filter(item=>item.nodeId==='details'&&item.status==='succeeded')).toHaveLength(2);
    const resumed=new AnalysisTaskScheduler(directory,async()=>config,()=>{},()=>batched);await resumed.initialize();await resumed.retry(created.id);const done=await terminal(resumed);
    expect(done.status,done.error).toBe('completed');expect([...calls.values()].sort()).toEqual([1,1,2]);
    expect(done.project.requirements).toHaveLength(25);expect(new Set(done.project.requirements.map(item=>item.id)).size).toBe(25);
  });
  ...
  it('超过旧硬上限的完整请求仍到达 Runtime 且只记录估算用量',async()=>{
    const prompts:string[]=[],directory=path.join(root,'unlimited-prompt'),value=project(),large='中'.repeat(21000)+'。';value.rawText=large;value.sourceUnits=[{id:'S-LARGE',label:'长需求',kind:'paragraph',excerpt:large,location:'第1段',status:'processed'}];
    const scheduler=new AnalysisTaskScheduler(directory,async()=>config,()=>{},()=>runtime(prompts));await scheduler.initialize();await scheduler.create(value);const done=await terminal(scheduler);
    expect(done.status,done.error).toBe('completed');expect(prompts.some(prompt=>estimateTokens(prompt)>16000)).toBe(true);expect(done.checkpoint?.promptMetrics?.some(metric=>metric.estimatedTokens>16000)).toBe(true);expect(done.checkpoint?.promptMetrics?.every(metric=>!('targetTokens' in metric)&&!('hardTokens' in metric))).toBe(true);
  });
  it('audit 执行失败不生成业务问题，重试只补跑失败工作',async()=>{
    const prompts:string[]=[],directory=path.join(root,'audit-retry'),base=runtime(prompts),auditCalls=new Map<string,number>(),value=project();value.rawText='字段 A 必填。字段 B 必填。';value.sourceUnits=[{id:'S-A',label:'A',kind:'paragraph',excerpt:'字段 A 必填。',location:'第1段',status:'processed'},{id:'S-B',label:'B',kind:'paragraph',excerpt:'字段 B 必填。',location:'第2段',status:'processed'}];
    const unstable:AnalysisRuntime={...base,promptAndWait:async(id,prompt,options)=>{const data=input(prompt);if(prompt.includes('“功能候选识别”')){prompts.push(prompt);return JSON.stringify({features:data.sourceUnits.map((unit:SourceUnit,index:number)=>({id:`LOCAL-F${index+1}`,name:`字段${index+1}`,kind:'function',appliesToFeatureIds:[],evidenceIds:data.evidenceCatalog.filter((e:any)=>e.sourceUnitId===unit.id).map((e:any)=>e.id)})),sourceDispositions:data.sourceUnits.map((unit:SourceUnit,index:number)=>({sourceUnitId:unit.id,contentRole:'requirement',reason:'明确要求',featureIds:[`LOCAL-F${index+1}`]}))})}if(prompt.includes('“产物依据核查”')){const featureId=data.feature.id as string,count=(auditCalls.get(featureId)??0)+1;auditCalls.set(featureId,count);if(featureId==='F-002'&&count===1){prompts.push(prompt);throw new Error('临时 Runtime 故障')}}return base.promptAndWait(id,prompt,options)}};
    const scheduler=new AnalysisTaskScheduler(directory,async()=>config,()=>{},()=>unstable);await scheduler.initialize();const created=await scheduler.create(value),failed=await terminal(scheduler);
    expect(failed.status).toBe('failed');expect(failed.project.audit).toBeUndefined();expect(failed.checkpoint?.auditIssues).toEqual([]);expect(failed.checkpoint?.auditWorkStates?.['F-001']).toMatchObject({state:'succeeded',attempts:1});expect(failed.checkpoint?.auditWorkStates?.['F-002']).toMatchObject({state:'failed',attempts:1,error:'临时 Runtime 故障'});expect(failed.checkpoint?.executionFailures?.at(-1)).toMatchObject({node:'audit',subjectId:'F-002',message:'临时 Runtime 故障'});
  ...
  it('细化首轮多个结构错误会一次反馈并在第二轮整体纠正',async()=>{
    ...
    const correction=prompts.find(item=>item.includes('previousCandidate'))??'';
    expect(correction).toContain('requirements.0.behavior');expect(correction).toContain('requirements.0.conditions.0');expect(correction).toContain('sourceUnitIds');expect(correction).toContain('previousCandidate');
