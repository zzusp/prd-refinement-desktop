...
export function acceptDirectDetails(requirementsValue:unknown,questionsValue:unknown,sourceUnits:SourceUnit[],requireEvidenceBindings=false){
  ...
  const requirements=requirementsValue.map((raw,index)=>{const item=raw as Record<string,unknown>,state=text(item.state,`requirements[${index}].state`);if(!reviewStates.has(state))throw new Error(`requirements[${index}].state 非法`);const requirement:RequirementDetail={id:text(item.id,`requirements[${index}].id`),title:text(item.title,`requirements[${index}].title`),behavior:text(item.behavior,`requirements[${index}].behavior`),conditions:strings(item.conditions,`requirements[${index}].conditions`),constraints:strings(item.constraints,`requirements[${index}].constraints`),explicitAcceptanceConditions:strings(item.explicitAcceptanceConditions,`requirements[${index}].explicitAcceptanceConditions`),sourceUnitIds:strings(item.sourceUnitIds,`requirements[${index}].sourceUnitIds`,false),ruleIds:[],state:state as RequirementDetail['state']};refs(requirement.sourceUnitIds,sourceIds,`requirements[${index}].sourceUnitIds`);
    ...
    if(rawBindings!==undefined){if(!rawBindings||typeof rawBindings!=='object'||Array.isArray(rawBindings))throw new Error(`${requirement.id}.evidenceBindings 必须是对象`);const binding=rawBindings as Record<string,unknown>,one=(value:unknown,label:string)=>{const selected=Array.isArray(value)?sourceRefs({sourceRefs:value},sourceUnits,label):value&&typeof value==='object'?(()=>{const entry=value as Record<string,unknown>;return sourceRefs(Array.isArray(entry.sourceRefs)?entry:{sourceRefs:[entry]},sourceUnits,label)})():sourceRefs({sourceUnitIds:value},sourceUnits,label);if(!selected.length)throw new Error(`${label} 不得为空`);return selected},many=(value:unknown,count:number,label:string)=>{if(!Array.isArray(value)||value.length!==count)throw new Error(`${label} 必须与对应字段逐项匹配：需要 ${count} 组，实际 ${Array.isArray(value)?value.length:'不是数组'} 组`);return value.map((entry,n)=>one(entry,`${label}[${n}]`))};requirement.evidenceBindings={behavior:one(binding.behavior,`${requirement.id}.evidenceBindings.behavior`),conditions:many(binding.conditions,requirement.conditions.length,`${requirement.id}.evidenceBindings.conditions`),constraints:many(binding.constraints,requirement.constraints.length,`${requirement.id}.evidenceBindings.constraints`),explicitAcceptanceConditions:many(binding.explicitAcceptanceConditions,requirement.explicitAcceptanceConditions.length,`${requirement.id}.evidenceBindings.explicitAcceptanceConditions`)}}
