...
将 Agent 交付包中的 CSV/Excel 工作表收敛为一份可直接勾选的 `agent-checklist.md`。它只承担逐项查漏，不替代原始 PRD，也不承载平台校验状态或代码验收结论。
...
- 多个原文位置用同一缩进层级逐条列出。
- 发布前从磁盘回读 `agent-checklist.md`，与确定性生成结果逐字比对，并纳入 manifest 哈希。
- 不再在 Agent 包中生成 `checklist.csv` 和 `checklist.xlsx`；`requirements.json` 保留为只读机器快照。

## 验证

- 中文标点、Markdown 符号和多行需求保持为一个可勾选事项。
- 范围排除后，Markdown 与内部需求快照的需求编号集合一致。
- 真实任务生成的新包以 `agent-checklist.md` 为唯一工作清单，不存在 CSV/Excel 工作清单。
