...
describe('需求细化数据契约', () => {
  ...
  it('完成任务默认进入功能范围工作台并集中任务动作',()=>{
    ...
    expect(html).toContain('选择筛选结果（1）');
    expect(html).not.toContain('标记本期不做');
    expect(html).not.toContain('恢复本期');
    expect(html).not.toContain('清除选择');
  ...
  it('旧版需求字段在全部需求中降级为只读清单而不触发白屏',()=>{
    ...
    expect(filtered).toContain('class="feature-filter-chip"');
    expect(filtered).toContain('<span>功能</span>');
    expect(filtered).toContain('审核列表');
    expect(filtered).toContain('1 条');
    expect(filtered).toContain('清除功能筛选：审核列表');
