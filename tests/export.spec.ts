...
it('Excel 仅保留七列需求清单与阅读说明，不输出历史问题或建议',async()=>{
 ...
 const wb=new ExcelJS.Workbook();await wb.xlsx.readFile(file);const main=wb.getWorksheet('需求清单')!;
 expect(wb.worksheets.map(sheet=>sheet.name)).toEqual(['需求清单','阅读说明']);
 ...
 for(const legacy of ['是否含税？','按含税金额','使用不含税金额','待处理事项','建议方案'])expect(values).not.toContain(legacy);
 expect(main.getCell('F2').dataValidation).toEqual(expect.objectContaining({type:'list',formulae:['"unchecked,checked,pending"']}));
