...
function runtime(mode:'question'|'repair-failure',calls:string[]):AnalysisRuntime{return {
 ...
 async executeOperation(request){
  ...
  else if(node==='submit_details')value={requirements:[{id:'LR',title:'填写名称',behavior:{text:'用户必须填写名称',evidenceIds:[input.evidenceCatalog[0].id]},conditions:[],constraints:[],explicitAcceptanceEvidenceIds:[]}],clarifications:mode==='question'?[{id:'LQ',level:'blocking',question:'是否允许重复名称？',reason:'原文要求确认',knownFacts:'名称必须填写',unresolvedPoint:'重复名称规则未明确',impact:'决定保存行为',levelReason:'保存规则必须确定',affectedIds:['LR'],evidenceIds:[input.evidenceCatalog.at(-1).id],resolutionProposal:{recommendation:'建议由业务明确是否允许名称重复后再实施保存校验',rationale:'避免自行确定未明确规则',impact:'影响保存校验',confirmation:'请确认是否允许名称重复',alternatives:[],evidenceIds:[input.evidenceCatalog.at(-1).id]}}]:[]};
...
describe('节点执行与交付边界集成',()=>{
 ...
 it('合法阻塞级业务澄清进入需求包，不成为平台失败',async()=>{
  const calls:string[]=[],scheduler=new AnalysisTaskScheduler(path.join(root,'questions'),async()=>config,()=>{},()=>runtime('question',calls));await scheduler.initialize();await scheduler.create(project());const done=await terminal(scheduler);
 ...
 it('修正通道故障保留原AuditIssue且不提交补丁或正式包',async()=>{
  ...
  const persisted=JSON.parse(await readFile(path.join(directory,`${created.id}.json`),'utf8'));expect(persisted.checkpoint.auditIssues[0].detail).not.toContain('认证');expect(persisted.project.requirements[0].behavior).toBe('用户必须填写名称');
 });
});
